...
LINE 圖文選單要「舞台」「展區」各佔一格,因此拆出兩個子頁網址;日文版另有一個入口:
...
| `/meet-greater-south-2026/ja/` | 日文版完整版(可加 `?view=stage` / `?view=zone`) |

`stage/`、`zone/`、`ja/` 底下的 `index.html` 都只是導向頁(自帶 `<title>` / OG,貼到 LINE 有各自的預覽標題),
會 `location.replace()` 到根目錄並帶上 `?view=stage` / `?view=zone` / `?lang=ja`,其他參數與 `#spot=` 原樣帶過去。
內容本體仍只有一份 `index.html`,不需要同步維護副本。

`index.html` 裡的 `VIEWS` 物件集中放兩個模式的預設開啟項目,文字在 `STR_ZH.views`(中文)與 `ja.ui.views`(日文);
...
右上角的「日本語 / 中文」鈕可以互切,切換會整頁重新載入並保留目前的 `?view=` 與 `#spot=`。
圖片(底圖、分區放大圖、社群預覽圖)兩個語言共用,不另做日文版。

- **語言判定**:`?lang=ja` / `?lang=zh` 參數優先;沒有參數時採用上次按語言鈕記住的偏好(`localStorage` 的 `mgs-lang`),
  所以從 LINE 圖文選單的 `stage/`、`zone/`(不帶參數)進來也會維持上次選的語言;都沒有就是中文。
- **字典只在需要時載入**:`<head>` 內的小段 script 判定為日文時才以 `document.write` 同步插入 `i18n/ja.js`(gzip 後約 85 KB),
  中文訪客完全不會下載。字典載入失敗時自動退回中文。
- **對照方式**:UI 字串以 key 覆蓋 `STR_ZH`;舞台、展區以 spot key 對照;節目名稱以中文標題對照;
  攤商以「展區 key + 中文顯示名稱」對照(攤位編號在主題館內不唯一,CX-02 有 21 家同編號)。
  對不到的項目一律維持中文,不會出現空白。
- **字體**:`<html lang="ja">` 時改用日文字體堆疊(Hiragino Sans / Noto Sans JP …),漢字才會是日本字形。
- **日文版的內部連結**(切換模式、分享出去的網址)都會帶 `lang=ja`;中文版的網址維持原樣。

日文內容的來源檔在 `data/i18n/ja/`,**不要直接改 `i18n/ja.js`**:

| 檔案 | 內容 |
| --- | --- |
| `data/i18n/ja/ui.json` | 介面字串(`ui`)、舞台名稱(`stages`)、展區名稱/介紹/重點(`zones`)、節目名稱(`schedule`,以中文標題為 key) |
| `data/i18n/ja/booths/<spot key>.json` | 各展區攤商:`zh` 為中文顯示名稱(對照用,須與 `BOOTHS` 完全相同)、`ja` 為日文顯示名稱(省略則沿用原名)、`desc` 為介紹 |
...
會重寫 `i18n/ja.js`,並列出尚未翻譯或對不到攤商的項目。**參展商名錄換版(重跑 `build-booth-data.py`)後也要跑一次**,
名稱改了或新增的攤商會出現在缺漏清單,補譯後再重跑即可;沒補的攤商在日文版會顯示中文介紹。
`index.html` 的 `?v=` 參數(`i18n/ja.js?v=…`)換版時記得一起改,舊訪客才不會拿到快取的舊字典。

攤商介紹的日文為翻譯稿,展區/舞台名稱與活動名稱沒有官方日文譯名時採意譯;公司名稱保留原文,必要時在 `ja` 附上英文名或日文說明(例:研究團隊、加速器)。
...
| 日文版字串與攤商介紹日譯 | `data/i18n/ja/`(產生 `i18n/ja.js`) |
...
跑完接著跑 `python3 tools/build-i18n.py`,看日文版有哪些攤商要補譯(見上方「日文版」)。
